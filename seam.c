...
int build_matrix(pixel_t** image_pixel_array, int *rows, int *cols){
    ...
    *cols = num_cols;
    ...
    // allocate a 2D array ;
    image_pixel_array = (pixel_t**) malloc(sizeof(pixel_t *) * num_rows);
    int i;
    for (i = 0; i < num_rows; i ++){
        image_pixel_array[i] = (pixel_t*)malloc(sizeof(pixel_t) * num_cols);
    }
    ...
    while (fgets(buf, 1500 , ppm_file) != NULL){
        ...
        while (sscanf(buf_ptr, "%d %d %d%n",  \
            ...
            (image_pixel_array[curr_row][curr_col]).R = R;
            (image_pixel_array[curr_row][curr_col]).G = G;
            (image_pixel_array[curr_row][curr_col]).B = B;
